...
public class CountrySort {
	...
	public static void main(String[] args) {
		...
		Comparator<String> longestName = (s1,s2) -> s2.length() - s1.length();      //it is updated to print the longest name of countries//
		...
		Comparator<String> reverseAlphabetic = (s1,s2) -> s2.compareToIgnoreCase(s1);   //it gives us the countries in reverse alphabetic//
		Arrays.sort(COUNTRIES,reverseAlphabetic);
		System.out.println("\nCountries in reverse alphabetic");
		for (int i=0; i<10;i++)
			System.out.println("\t"+COUNTRIES[i]);
		
		// Task 3: Print Top 10 list of country names containing most As (either A or a). 
		Comparator<String> countAs = (s1,s2) -> containsA(s1,s2);       //it gives us the countries with most A or a//
	...
	public static int containsA(String s1, String s2) { //compare between two strings how many A or a in each string//
		int countS1 = 0;
		int countS2 = 0;

		for (int i = 0; i < s1.length(); i++) { //calculate A and a in the first string//
			if (s1.charAt(i) == 'A' || s1.charAt(i) == 'a') {
				countS1++;
			}
		}
		for (int i = 0; i < s2.length(); i++) { //calculate A and a in the second string//
			if (s2.charAt(i) == 'A' || s2.charAt(i) == 'a') {
				countS2++;
			}
		}
		return countS2-countS1;  //difference between number of A or a for each string// 
	}
